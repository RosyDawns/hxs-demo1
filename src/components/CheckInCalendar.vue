  ...
  <div class="bg-white p-4 mb-3 rounded-lg">
    <div class="mb-3 flex items-center justify-between">
      ...
      <div class="flex-1 flex items-center justify-center">
        ...
        <span class="text-sm">{{ currentYear }}年{{ currentMonth }}月</span>
      ...
      <div class="flex-1 flex justify-end">
        <button
          @click="toggleExpand"
          class="px-2 py-1 text-xs text-gray-400 border border-gray-400 rounded-full hover:text-primary-dark transition-colors"
        >
          {{ expanded ? "收起日历" : "展开日历" }}
        </button>
      </div>
    ...
    <div
      ...
      :style="{ maxHeight: expanded ? '1000px' : calendarHeight }"
    >
      <div class="grid grid-cols-7 gap-2">
        ...
        <div
          ...
          @click="selectDay(day)"
        >
          <i v-if="isChecked(day)" class="fa fa-check text-xs mb-1"></i>
          <span v-if="!isChecked(day)">{{ day }}</span>
          <i
            class="fa-solid fa-volleyball"
            :class="{
              'text-green-400': isChecked(day),
            }"
          ></i>
        </div>
  ...
  setup(props, { emit }) {
    ...
    const displayNextMonthDays = computed(() => {
      ...
      const totalDisplayed =
        displayPrevMonthDays.value.length +
        displayCurrentMonthDays.value.length;
    ...
    const getDayClass = (day) => {
      ...
      if (isToday && checked) {
        ...
        return "border-1 border-primary text-primary font-bold";
      } else {
