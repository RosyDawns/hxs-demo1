<template>
  <div class="p-4">
    <!-- 运动打卡宣传卡片 -->
    <div class="bg-gradient-to-r from-yellow-400 to-green-500 rounded-xl overflow-hidden mb-4">
      <div class="p-6 text-white">
        <h3 class="text-2xl font-bold mb-2">你的每一滴汗水都是脂肪的眼泪</h3>
      </div>
      <img src="https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=600" alt="运动" class="w-full h-48 object-cover">
      <div class="p-4 bg-gradient-to-r from-yellow-400 to-green-500">
        <div class="flex justify-between items-center text-white">
          <span class="text-xl font-bold">YOUR BEST TRAINING</span>
          <button class="bg-yellow-500 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-yellow-600 transition-colors">
            LEARN MORE
          </button>
        </div>
      </div>
    ...
    <div class="grid grid-cols-4 gap-3 mb-4">
      <div 
        ...
        :class="selectedExerciseType === exercise.type ? 'ring-2 ring-primary' : ''"
        class="flex flex-col items-center nav-action cursor-pointer transition-all">
        <div :class="exercise.bgColor" class="w-16 h-16 rounded-lg flex items-center justify-center mb-1">
          <i :class="exercise.icon" class="text-2xl" :style="{ color: exercise.color }"></i>
        </div>
    ...
    <!-- 已选择的运动信息 -->
    <div v-if="selectedExerciseType" class="bg-white rounded-xl p-4 mb-4 card-shadow">
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <label class="text-xs text-gray-500 mb-1 block">消耗 (kcal)</label>
            <input 
        ...
        <div>
          ...
          <div class="flex gap-2">
            <button 
              ...
              :class="intensity === level.value ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600'"
      ...
      <!-- 运动照片上传 -->
      <div class="mt-3">
        <label class="text-xs text-gray-500 mb-1 block">运动照片（可选）</label>
        ...
        <div v-else class="relative">
          <img :src="uploadedImage" alt="运动照片" class="w-full h-48 object-cover rounded-lg">
          <button @click="removeImage" class="absolute top-2 right-2 bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600">
    ...
    <div class="bg-white rounded-xl p-4 mb-4 card-shadow">
      <h4 class="font-bold text-sm mb-3">今日运动统计</h4>
      <div class="grid grid-cols-3 gap-3 text-center">
        <div>
          <div class="text-2xl font-bold text-primary">{{ totalDuration }}</div>
          <div class="text-xs text-gray-500">总时长 (分钟)</div>
        </div>
        <div>
          <div class="text-2xl font-bold text-orange-500">{{ totalCaloriesBurned }}</div>
          <div class="text-xs text-gray-500">消耗 (kcal)</div>
        </div>
        <div>
          <div class="text-2xl font-bold text-green-500">{{ exerciseCount }}</div>
          <div class="text-xs text-gray-500">运动次数</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    const exerciseTypes = [
      { type: 'custom', label: '自定义', icon: 'fa fa-ruler', color: '#f97316', bgColor: 'bg-orange-50' },
      { type: 'gym', label: '健身', icon: 'fa fa-dumbbell', color: '#06b6d4', bgColor: 'bg-cyan-50' },
      { type: 'ball', label: '球类运动', icon: 'fa fa-basketball-ball', color: '#f97316', bgColor: 'bg-orange-50' },
      { type: 'outdoor', label: '户外运动', icon: 'fa fa-running', color: '#3b82f6', bgColor: 'bg-blue-50' }
    ]
    ...
    const canSubmit = computed(() => {
      return selectedExerciseType.value && exerciseName.value && duration.value > 0
    })
    ...
    const getExerciseTypeLabel = () => {
      const exercise = exerciseTypes.find(e => e.type === selectedExerciseType.value)
    ...
    const calculateCalories = () => {
      if (selectedExerciseType.value && duration.value > 0) {
        ...
        const intensityMultiplier = intensity.value === 'low' ? 0.8 : intensity.value === 'high' ? 1.2 : 1
        caloriesBurned.value = Math.round(baseRate * duration.value * intensityMultiplier)
      }
    }
