<template>
  <div class="bg-white rounded-2xl p-4 shadow-sm mb-3 relative">
    <!-- TOP标签 -->
    <div
      v-if="rank <= 3"
      class="absolute left-2 top-0 w-10 h-10 flex flex-col items-center justify-center text-white font-bold rounded-b-lg z-10"
    ...
    <div class="flex items-start">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center justify-between mb-2">
          ...
          <button
            class="px-3 py-1 text-white text-xs rounded-full whitespace-nowrap"
        ...
        <p class="text-xs text-gray-600 mb-2">擅长：{{ master.specialty }}</p>
        <div class="flex items-center justify-between">
          ...
          <!-- 去打Call按钮 -->
          <button
            class="flex items-center space-x-1 px-3 py-0.5 border border-orange-500 text-orange-500 rounded-full hover:bg-orange-50 transition-colors"
            @click="handleCall"
          >
            <span class="text-xs">去打Call</span>
          </button>
        </div>
      </div>
    </div>
