<template>
  <div class="p-4">
    <!-- 饮食打卡宣传卡片 -->
    <div class="bg-gradient-to-r from-green-400 to-green-600 rounded-xl p-6 text-white mb-4">
      <h3 class="text-2xl font-bold mb-2">饮食低热量，成功一大半</h3>
      <div class="flex items-center mt-4">
        <img src="https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400" alt="健康饮食" class="w-full h-40 object-cover rounded-lg">
      </div>
      <div class="mt-4 bg-white/20 rounded-lg p-3 backdrop-blur-sm">
        <div class="flex justify-between items-center">
          <span class="text-sm">HEALTHY TASTE</span>
          <span class="text-xl font-bold">45% OFF</span>
        </div>
        <button class="mt-2 bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-bold w-full hover:bg-green-800 transition-colors">
          ORDER NOW
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-4 gap-3 mb-4">
      <div 
        ...
        <div :class="meal.bgColor" class="w-16 h-16 rounded-full flex items-center justify-center text-3xl mb-1">
          {{ meal.emoji }}
        </div>
    ...
    <!-- 已选择的餐食信息 -->
    <div v-if="selectedMealType" class="bg-white rounded-xl p-4 mb-4 card-shadow">
      ...
      <div class="mb-4">
        ...
        <div v-else class="relative">
          <img :src="uploadedImage" alt="食物照片" class="w-full h-48 object-cover rounded-lg">
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="text-xs text-gray-500 mb-1 block">热量 (kcal)</label>
            <input 
      ...
      <div class="mt-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button 
            ...
            :key="tag"
            @click="toggleTag(tag)"
            :class="selectedTags.includes(tag) ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600'"
    ...
    <div class="bg-white rounded-xl p-4 mb-4 card-shadow">
      <h4 class="font-bold text-sm mb-3">今日饮食统计</h4>
      <div class="grid grid-cols-3 gap-3 text-center">
        <div>
          <div class="text-2xl font-bold text-primary">{{ totalCalories }}</div>
          <div class="text-xs text-gray-500">总热量 (kcal)</div>
        </div>
        <div>
          <div class="text-2xl font-bold text-green-500">{{ mealsCount }}</div>
          <div class="text-xs text-gray-500">已记录餐次</div>
        </div>
        <div>
          <div class="text-2xl font-bold text-orange-500">{{ remainingCalories }}</div>
          <div class="text-xs text-gray-500">剩余热量</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    const mealTypes = [
      { type: 'breakfast', label: '+早餐', emoji: '🍳', bgColor: 'bg-yellow-50' },
    ...
    const nutritionTags = ['高蛋白', '低脂肪', '低碳水', '高纤维', '有机食品', '素食']
