<template>
  <div class="daily-coach bg-gray-50">
    <!-- 筛选标签 -->
    <div class="bg-white px-4 py-3 flex items-center space-x-2 overflow-x-auto">
      <button 
        v-for="filter in filters" 
        :key="filter.id"
        :class="activeFilter === filter.id ? 'bg-primary text-white' : 'bg-gray-100 text-gray'"
        class="px-4 py-1.5 rounded-full text-xs whitespace-nowrap flex items-center"
        @click="activeFilter = filter.id"
      >
        {{ filter.name }}
        <i v-if="filter.hasDropdown" class="fa-solid fa-angle-down ml-1 text-xs"></i>
      </button>
...
export default {
  ...
  methods: {
    ...
    handleViewProfile(coachId) {
      ...
      // TODO: 跳转到唤醒师详情页
      // this.$router.push(`/coach/${coachId}`)
    }
  }
}
</script>
