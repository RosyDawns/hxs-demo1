  ...
  <div class="daily-home bg-gray-50">
    ...
    <div class="bg-white px-5 pt-6 pb-5">
      <div class="flex items-start justify-between mb-4">
        <div class="flex-1">
          <div class="flex items-baseline">
            <span class="text-[64px] font-bold text-primary leading-none">{{ targetWeight }}</span>
            <span class="text-xl font-medium text-dark ml-1">kg</span>
          </div>
          <div class="text-dark text-base mt-2">目标体重</div>
        </div>
        <div class="flex flex-col items-end">
          <img src="@/assets/images/img-12.jpg" alt="用户头像" class="w-20 h-20 rounded-full mb-2">
          <div class="text-sm flex items-baseline">
            <span class="text-gray-500">剩余唤醒周期：</span>
            <span class="text-primary font-bold text-2xl mx-1">{{ remainingDays }}</span>
      ...
      <div class="flex items-center gap-3 mb-4">
        <div class="flex-1 bg-gray-50 rounded-2xl px-4 py-3">
          ...
          <div class="text-sm text-primary font-medium">{{ lastWeightDate }}</div>
        </div>
        <div class="flex-1 bg-gray-50 rounded-2xl px-4 py-3">
    ...
    <div class="bg-white mt-2.5 px-5 py-4">
      <div class="flex items-center justify-between mb-3">
        ...
        <button class="text-primary text-sm flex items-center" @click="showDeclarationHistory">
    ...
    <div class="bg-white mt-2.5 px-5 pt-4 pb-8">
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-baseline">
          ...
          <span class="text-[40px] font-bold text-primary leading-none">{{ todayScore }}</span>
        </div>
      </div>

      <!-- 打卡进度圈 -->
      <div class="relative flex items-center justify-center" style="height: 340px; margin-top: 20px;">
        <!-- SVG 圆环 -->
        <svg class="absolute" width="220" height="220" viewBox="0 0 220 220" style="top: 60px;">
          <defs>
          ...
          <!-- 背景圆环 -->
          <circle cx="110" cy="110" r="85" fill="none" stroke="#E8E8E8" stroke-width="14"/>
        ...
        <!-- 打卡标签 - 饮食打卡 (左上) -->
        <div class="absolute cursor-pointer" style="top: 10px; left: 10px;" @click="handleCheckIn('diet')">
          <div class="bg-gradient-to-r from-yellow-400 to-yellow-500 text-white px-4 py-1.5 rounded-full text-xs font-medium shadow-md">
            饮食打卡
          </div>
          <div class="bg-yellow-50 text-yellow-600 px-3 py-1 rounded-full text-xs mt-1.5 text-center">
            已打卡
          </div>
        </div>

        <!-- 打卡标签 - 心情打卡 (右上) -->
        <div class="absolute cursor-pointer" style="top: 10px; right: 10px;" @click="handleCheckIn('mood')">
          <div class="bg-gradient-to-r from-pink-400 to-pink-500 text-white px-4 py-1.5 rounded-full text-xs font-medium shadow-md">
            心情打卡
          </div>
          <div class="bg-pink-50 text-pink-400 px-3 py-1 rounded-full text-xs mt-1.5 text-center">
            已打卡
          </div>
        </div>

        <!-- 打卡标签 - 运动打卡 (左下) -->
        <div class="absolute cursor-pointer" style="bottom: 10px; left: 10px;" @click="handleCheckIn('exercise')">
          <div class="bg-gradient-to-r from-yellow-400 to-yellow-500 text-white px-4 py-1.5 rounded-full text-xs font-medium shadow-md">
            运动打卡
          </div>
          <div class="bg-yellow-50 text-yellow-600 px-3 py-1 rounded-full text-xs mt-1.5 text-center">
        ...
        <!-- 打卡标签 - 睡眠打卡 (左下位置，调整) -->
        <div class="absolute cursor-pointer" style="bottom: 70px; left: 0;" @click="handleCheckIn('sleep')">
          <div class="bg-gradient-to-r from-orange-400 to-orange-500 text-white px-4 py-1.5 rounded-full text-xs font-medium shadow-md">
            睡眠打卡
          </div>
          <div class="bg-orange-50 text-orange-500 px-3 py-1 rounded-full text-xs mt-1.5 text-center">
            已打卡
          </div>
        </div>

        <!-- 打卡标签 - 代谢打卡 (右下) -->
        <div class="absolute cursor-pointer" style="bottom: 70px; right: 0;" @click="handleCheckIn('metabolism')">
          <div class="bg-gradient-to-r from-pink-400 to-pink-500 text-white px-4 py-1.5 rounded-full text-xs font-medium shadow-md">
            代谢打卡
          </div>
          <div class="bg-gray-100 text-gray-400 px-3 py-1 rounded-full text-xs mt-1.5 text-center">
            未打卡
          </div>
        </div>
      </div>
