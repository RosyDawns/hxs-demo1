<template>
  <div class="page flex flex-col h-screen bg-gray-50" id="page-checkin">
    ...
    <header class="sticky top-0 z-50 bg-white border-b border-gray-100 p-3 flex items-center">
      ...
      <h2 class="font-bold">{{ activeTabName }}</h2>
      <div class="flex-1 flex justify-end">
        <button class="text-gray-500 text-sm" @click="showHistory">打卡记录</button>
      </div>
    </header>
    ...
    <div class="bg-white border-b border-gray-100">
      <div class="flex overflow-x-auto px-3">
        <button 
          ...
          :class="activeTab === tab.id ? 'text-primary border-b-2 border-primary font-bold' : 'text-gray-500'"
          class="px-4 py-3 whitespace-nowrap text-sm transition-colors">
          {{ tab.name }}
        </button>
      </div>
    </div>
    ...
    <transition name="fade">
      <div v-if="showToast" class="fixed top-20 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50">
